...
public class Consultation : AuditableEntity
{
    ...
    // ✅ REMOVED: PaymentId was never the actual FK — the real relationship
    // is configured the other way (Payment.ConsultationId is the FK, see
    // ConsultationConfiguration's HasOne(c => c.Payment).WithOne(...)
    // .HasForeignKey<Payment>(p => p.ConsultationId)). This property was
    // just a floating, never-populated column.
