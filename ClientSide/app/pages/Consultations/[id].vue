<template>
  <div class="h-screen flex flex-col bg-gray-50" dir="rtl">
    ...
    <header class="bg-white border-b border-emerald-100 px-6 py-4 flex items-center justify-between shadow-sm">
      <div class="flex items-center gap-4">
        ...
        <div class="flex items-center gap-3">
          <div class="relative">
            <img
                :src="consultation?.otherUserImageUrl || 'https://via.placeholder.com/40'"
                :alt="consultation?.otherUserName"
                class="w-10 h-10 rounded-full object-cover border-2 border-amber-400"
            />
            <!-- Online Indicator -->
            <span
                v-if="consultation?.isOnline"
                class="absolute bottom-0 right-0 w-3 h-3 bg-green-500 border-2 border-white rounded-full"
            ></span>
          </div>
          <div>
            <h3 class="font-bold text-emerald-900" style="font-family: 'Amiri', serif;">
              {{ consultation?.otherUserName }}
            </h3>
            <p class="text-xs text-gray-500">
              {{ consultation?.otherUserRole === 'Lawyer' ? 'محامي' : 'عميل' }}
            </p>
          </div>
        </div>
      </div>
    ...
    <main class="flex-1 flex overflow-hidden">
      ...
      <!-- Sidebar: Consultation Info -->
      <aside class="w-80 bg-white border-r border-emerald-100 p-6 overflow-y-auto hidden lg:block">
        <h4 class="font-bold text-emerald-900 mb-4" style="font-family: 'Amiri', serif;">
          تفاصيل الاستشارة
        </h4>
        ...
        <div class="space-y-4 text-sm">
          <div class="flex justify-between">
            ...
            <span class="font-medium">{{ formatDate(consultation?.scheduledAt) }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500">المدة:</span>
            <span class="font-medium">{{ consultation?.durationMinutes }} دقيقة</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-500">الحالة:</span>
            <span class="font-medium" :class="getStatusColor(consultation?.status)">
              {{ getStatusText(consultation?.status) }}
            </span>
          </div>
        </div>
        ...
        <div class="mt-6 pt-6 border-t border-emerald-100">
          <h5 class="font-bold text-emerald-900 mb-2 text-xs">آخر رسالة</h5>
          <p class="text-xs text-gray-600 truncate">
            {{ consultation?.lastMessageContent || 'لا توجد رسائل' }}
          </p>
          <p class="text-xs text-gray-400 mt-1">
            {{ formatTime(consultation?.lastMessageDate) }}
          </p>
        </div>
      </aside>
    </main>
    ...
    <footer class="bg-white border-t border-emerald-100 px-6 py-4">
      <div class="flex items-center gap-3">
        <textarea
            ...
            rows="1"
            class="flex-1 resize-none rounded-xl border border-emerald-200 bg-emerald-50/50 px-4 py-3 text-sm focus:border-emerald-500 focus:outline-none focus:ring-2 focus:ring-emerald-500/20"
            style="font-family: 'Amiri', serif;"
        ></textarea>
        <button
            @click="sendMessage"
            :disabled="!newMessage.trim() || !isConnected"
            class="px-6 py-3 bg-gradient-to-l from-amber-400 to-amber-500 text-white rounded-xl font-medium hover:from-amber-500 hover:to-amber-600 disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-md"
        >
          إرسال
        </button>
      </div>
...
// Connect to SignalR Hub (reuse your useSignalRChat composable)
const connectToChat = async () => {
  // Use the composable we created earlier
  const { startConnection, sendMessage: sendSignalRMessage } = useSignalRChat(
      consultationId,
      Number(useCookie('user_id').value)
  );
...
const sendMessage = async () => {
  ...
  // This will be handled by your useSignalRChat composable
  newMessage.value = '';
};
...
const startVideoCall = () => {
  // Navigate to video call page or open modal
