...
export const useAdmin = () => {
    ...
    // ── Votes ──
    const getVotes = (includeResolved = false) =>
        $fetch<PagedResult<AdminVote>>(`${base}/admin/votes`, { query: { includeResolved }, headers: h() })
    ...
    const getNotifications = (unreadOnly = false) =>
        $fetch<AppNotification[]>(`${base}/notifications`, { query: { unreadOnly }, headers: h() })
    ...
    return {
        getStats, getChart, getRecentActivities,
        getUsers, getContactLog, getUserChart, proposeBan,
        getPendingLawyers, getLawyerChart, proposeVerification, proposeUnverification,
        getVotes, castVote, deletePost,
