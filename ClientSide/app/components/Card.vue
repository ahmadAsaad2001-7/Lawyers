  ...
  <div class="bg-white rounded-2xl p-5 border border-emerald-900/10 shadow-sm hover:shadow-md transition-shadow relative flex flex-col justify-between min-h-[380px]">
    ...
    <div v-else class="flex flex-col space-y-4 text-right">
      <div class="bg-emerald-50/50 p-3 rounded-xl border border-emerald-100">
        <h4 class="font-bold text-sm text-emerald-900" style="font-family: 'Amiri', serif;">{{ lawyer.lawFirmName || 'مكتب المحاماة' }}</h4>
        <p class="text-xs text-gray-600 mt-1">
          📍 {{ lawyer.address || [lawyer.city, lawyer.state].filter(Boolean).join('، ') || 'غير محدد' }}
        </p>
        <p v-if="lawyer.phone" class="text-xs text-gray-600 mt-0.5">📞 {{ lawyer.phone }}</p>
        <p class="text-xs text-gray-700 mt-2 line-clamp-3 leading-relaxed">{{ lawyer.bio }}</p>
      </div>

      <div>
        <span class="text-xs font-bold text-emerald-900 block mb-1.5">مجال الاختصاص:</span>
        <div class="flex flex-wrap gap-1.5">
          <span class="bg-gray-100 text-gray-700 text-[11px] px-2 py-0.5 rounded-md">
            {{ lawyer.specialization || 'غير محدد' }}
          </span>
        </div>
      </div>

      <div v-if="lawyer.languages?.length">
