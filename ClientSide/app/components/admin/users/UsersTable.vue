  ...
  <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
    <table class="w-full text-sm">
      ...
      <tr v-for="u in users" :key="u.id" class="border-t border-gray-100 hover:bg-gray-50">
        ...
        <td class="px-4 py-3 text-gray-600" dir="ltr">{{ u.phoneNumber || '—' }}</td>
        <td class="px-4 py-3 text-gray-600">{{ u.consultationCount }}</td>
        <td class="px-4 py-3 text-gray-600">{{ new Date(u.createdAt).toLocaleDateString('en-GB') }}</td>
        <td class="px-4 py-3">
        ...
        <td class="px-4 py-3">
          <div class="flex justify-end gap-2">
            <button class="rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs font-medium text-gray-600 hover:bg-gray-100" @click="emit('view', u)">View</button>
